...
emqx_home=/usr/local/emqx
${emqx_home}/bin/emqx start

#tail -f ${emqx_home}/log/emqx.log.1 &

# Wait and ensure emqx status is running

############
WAIT_TIME=0
while [[ -z "$(${emqx_home}/bin/emqx_ctl status |grep 'is running'|awk '{print $1}')" ]]
do
    sleep 1
...
do  
    ...
    if [[ ! -z "$(${emqx_home}/bin/emqx_ctl status |grep 'is running'|awk '{print $1}')" ]]; then
        IDLE_TIME=0
        # Print the latest emqx.log
        if [[ -f ${emqx_home}/log/emqx.log.${NEW_LOG_NUM} ]];then
            tail -f ${emqx_home}/log/emqx.log.${NEW_LOG_NUM} &
            [[ ! -z $(ps -ef |grep "tail -f ${emqx_home}/log/emqx.log" | grep -vE "grep|emqx.log.${NEW_LOG_NUM}" | awk '{print $1}') ]] && kill $(ps -ef |grep "tail -f ${emqx_home}/log/emqx.log" | grep -vE "grep|emqx.log.${NEW_LOG_NUM}" | awk '{print $1}') 
